...
def image_stream(datapath, image_size=[320, 512], stereo=False, stride=1): #.../glass
    ...
    map_l = cv2.initUndistortRectifyMap(K_l, np.zeros(5), R_l, P_l[:3,:3], (raw_size[0], raw_size[1]), cv2.CV_32F) #需要W,H [:3,:3]
    map_r = cv2.initUndistortRectifyMap(K_r, np.zeros(5), R_r, P_r[:3,:3], (raw_size[0], raw_size[1]), cv2.CV_32F)
    # T_l43 = np.vstack((R_l, np.array([0,0,0])))
    # T_l = np.hstack(T_l43, np.array([0,0,0,1])) #Trect_l
    # T_l_inv = np.linalg.inv(T_l) #Tl_rect
    # T_Brect = T_Bl.dot(T_l_inv) # 矫正相机系 到 imu(gt)
...
if __name__ == '__main__':
    ...
    # np.savetxt(os.path.join(outquatdir,seqname+'.csv'),traj_imu_time,delimiter=' ') # 还有time也保存  对于ape 用空格
    ...
    #             scale=True, title=seqname) #/home/dlr/anaconda3/envs/droidenv5/lib/python3.9/site-packages/numpy/core/fromnumeric.py:3440: RuntimeWarning: Mean of empty slice.
