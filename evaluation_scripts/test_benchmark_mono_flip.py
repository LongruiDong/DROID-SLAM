...
def image_stream(folder_path, intrinsics_vec, distortion_vec,image_size=[384, 512], stereo=False, stride=1):
    ...
    for t, img in enumerate(images):
        ...
        intrinsics_flip = torch.rand_like(intrinsics)
        intrinsics_flip[0] = intrinsics[1]
        intrinsics_flip[1] = intrinsics[0]
        intrinsics_flip[2] = intrinsics[3]
        intrinsics_flip[3] = intrinsics[2]
        yield t, image[None], intrinsics_flip
...
if __name__ == '__main__':
    ...
    parser.add_argument("--image_size", default=[512,384])
    ...
    # 保存结果
    outquatdir = 'result_filp'
    folder = os.path.exists(outquatdir)
    if not folder:                   #判断是否存在文件夹如果不存在则创建为文件夹
        os.makedirs(outquatdir)
    seqname = args.datapath.split('/')[-3]+"-"+args.datapath.split('/')[-2]
    np.savetxt(os.path.join(outquatdir,seqname+'-mono_android.csv'),traj_est_time,delimiter=' ') # 还有time也保存
    traj_ref0 = np.loadtxt(args.gt, delimiter=',')[:, :8] #t wxyz
    np.savetxt(os.path.join(outquatdir,seqname+'-monogt_android.csv'),traj_ref0,delimiter=',')
    ...
    results0 = evaluator.evaluate_one_trajectory(traj_ref_1, traj_est_1, #也会画轨迹
                scale=True, title=seqname+'-mono_android_flip')
    print(results0)
