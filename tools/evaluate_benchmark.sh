...
for ((i=0; i<5; i++)); do
    ...
    # echo "$cudaid"
    # --stereo
    python evaluation_scripts/test_glass.py --datapath=$BENCHMARK_PATH/${evalset[$i]}/glass --gt=$BENCHMARK_PATH/gt_result2022/${evalset[$i]}/glass/gba_pose.csv --weights=droid.pth --disable_vis --buffer=${bufferset[$i]} --plot_curve > logarmono/seq-${nameset[$i]}.log
