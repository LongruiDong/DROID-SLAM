...
for ((i=0; i<21; i++)); do
    CUDA_VISIBLE_DEVICES=2,3 python evaluation_scripts/test_benchmark_mono.py --datapath=$BENCHMARK_PATH/${evalset[$i]}/$CAMERA --gt=$BENCHMARK_PATH/gt_result2022/${evalset[$i]}/$CAMERA/gba_pose.csv --seq_name=$CAMERA/${evalset[$i]} --disable_vis --buffer=${bufferset[$i]} > log_mono_ios/seq-outdoor$i.log $@
done